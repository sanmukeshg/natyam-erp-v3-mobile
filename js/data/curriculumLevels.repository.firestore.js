/**
 * NATYAM ERP 2.0 — Curriculum levels repository (Firestore)
 *
 * Milestone 23: part of the Settings reference data migration. Keeps the
 * exact external shape the old IndexedDB-backed CurriculumLevelRepository
 * had — find/findOrFail/all/where/count/search/create/update/save/
 * remove/restore/createMany, plus active()/ordered(). Only importer:
 * curriculum.service.js. No create/update/delete path exists anywhere in
 * the app today — levels are populated only by a historical IndexedDB
 * SCHEMA migration step and read thereafter as a dropdown vocabulary;
 * create()/update()/remove() exist here for interface parity only,
 * matching every prior migration's convention for an unused-but-complete
 * repository surface.
 */
...
const COLLECTION_NAME = 'curriculumLevels';
const curriculumLevelsCollection = collection(firestore, COLLECTION_NAME);
...
async function writeAuditRow(action, id, detail) {
    await recordAuditEntry('Curriculum level', action, id, detail);
}

class FirestoreCurriculumLevelRepository {
    /* ---------------------------------------------------------------- HOOKS */
    ...
    async findOrFail(id) {
        if (!id) throw new Error('No curriculum level was specified.');
        const record = await this.find(id);
        if (!record) throw new Error(`Curriculum level ${id} no longer exists.`);
        return record;
    }

    async all({ includeDeleted = false } = {}) {
        const snap = await getDocs(curriculumLevelsCollection);
        const rows = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        return includeDeleted ? rows : rows.filter(visible);
    }

    async where(index, value) {
        const snap = await getDocs(query(curriculumLevelsCollection, where(index, '==', value)));
    ...
    async create(data) {
        ...
        const ref = await addDoc(curriculumLevelsCollection, full);
    ...
    async restore(id) {
        const snap = await getDoc(doc(firestore, COLLECTION_NAME, id));
        if (!snap.exists()) throw new Error(`Curriculum level ${id} no longer exists.`);
        await updateDoc(doc(firestore, COLLECTION_NAME, id), {
            deletedAt: null,
            deletedBy: null,
            updatedAt: nowISO()
    ...
    async createMany(items) {
        ...
        for (const item of items) {
            ...
            const ref = doc(curriculumLevelsCollection);
            batch.set(ref, full);
...
export const curriculumLevels$ = new FirestoreCurriculumLevelRepository();
